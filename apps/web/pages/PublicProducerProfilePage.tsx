  ...
  return (
    <main className="min-h-screen bg-[#f6f0dd] pb-12">
      ...
      <div className="mx-auto max-w-6xl space-y-8 px-4 py-8">
        ...
        {Array.isArray(farm.photos) && farm.photos.length > 0 && <section><h2 className="text-3xl font-serif">Photos</h2><div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3">{farm.photos.map((photo: AnyDoc, index: number) => <img key={`${photo.url}-${index}`} src={photo.url} alt={photo.alt || `${farm.farmName} photo ${index + 1}`} loading="lazy" decoding="async" className="h-48 w-full rounded-2xl object-cover shadow sm:h-64" />)}</div></section>}

        <section><h2 className="text-3xl font-serif">Products</h2>{visibleProducts.length === 0 ? <p className="mt-4 rounded-2xl bg-white p-6 text-stone-600">No products are currently available.</p> : <div className="mt-4 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{visibleProducts.map((product) => { const discount = activeProductDiscount(product); return <article key={product.id} className="mfm-deferred-card overflow-hidden rounded-2xl bg-white shadow">{product.imageUrl ? <img src={product.imageUrl} alt={product.title} loading="lazy" decoding="async" className="h-48 w-full object-cover" /> : <div className="h-40 bg-stone-200" />}<div className="p-5">{discount && <span className="rounded-full bg-red-100 px-2 py-1 text-xs font-black text-red-800">{discount.percent}% OFF</span>}<h3 className="mt-2 text-xl font-bold">{product.title}</h3><p className="mt-2 text-sm text-stone-600">{product.description}</p><div className="mt-3 font-black text-emerald-900">${(effectiveProductPriceCents(product) / 100).toFixed(2)} / {product.unit || "each"}{discount && <span className="ml-2 text-sm font-normal text-stone-400 line-through">${(discount.originalPriceCents / 100).toFixed(2)}</span>}</div><Link to="/buyer" className="mt-4 inline-block rounded-xl bg-emerald-900 px-4 py-2 font-bold text-white">Shop in market</Link></div></article>;})}</div>}</section>
